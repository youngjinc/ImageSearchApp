...
class SearchPagingSource(private val searchService: SearchService, private val keyword: String) :
    ...
    private var isEndCache: Boolean = false
    private var cache = mutableListOf<Content>()
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Content> {
        return try {
            ...
            val nextKey = if (isEndCache) null else page + 1
     ...
     * 나머지 콘텐츠는 cache에 저장해두고 다음 페이지에 대한 이미지, 비디오 검색결과를 합해서 위 과정을 반복하여 최신순으로 두 리스트를 정렬한다.
    ...
    private fun getLatestContentByPageSize(): List<Content> {
        // 이미지와 동영상 중 어느 하나라도 검색 결과가 존재한다면 임시 저장소에 추가하고, cache의 요소를 최신순으로 정렬
        if (imageList.isNotEmpty() || videoList.isNotEmpty()) {
            cache.addAll(imageList + videoList)
            cache.sortByDescending { it.dateTime }
        }

        // 현재 PagingData로 내보낼 최신 콘텐츠로, 갯수는 페이지 사이즈에 해당
        val currentContents =
            if (cache.size >= PAGE_SIZE) {
                cache.subList(0, PAGE_SIZE - 1)
            } else {
                isEndCache = true
                cache
            }

        // 내보낼 최신 콘텐츠를 cache에서 제거
        cache =
            if (currentContents.size == PAGE_SIZE)
                cache.subList(PAGE_SIZE, cache.size - 1)
            else mutableListOf()
